...
class complex_GRU_conv(torch.nn.Module):
    def __init__(self, num_input, num_hidden, num_layers, bias=False):

        super(complex_GRU_conv, self).__init__()
        ...
        self.FCLr      = torch.nn.Linear(in_features=num_hidden, out_features=num_hidden, bias=False)
        self.FCLi      = torch.nn.Linear(in_features=num_hidden, out_features=num_hidden, bias=False)
        self.act       = nn.LeakyReLU(0.1)


    def forward(self, xr, xi, seq_L=-1, batch=-1):
        originalShape = xr.shape

        xr            = torch.reshape(xr, [batch, seq_L, -1])
        xi            = torch.reshape(xi, [batch, seq_L, -1])
        ...
        xr            = torch.reshape(xr, [originalShape[0], originalShape[1], originalShape[2], originalShape[3]])
        xi            = torch.reshape(xi, [originalShape[0], originalShape[1], originalShape[2], originalShape[3]])
